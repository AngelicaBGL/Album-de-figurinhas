...
void comprar_envelope(Figurinha *figurinha, Album *album)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        char opcao;
        int aux = 0;
        int num = rand() % 679; // gerar um numero de 0 a 9
        printf("FIGURINHA %d:\n", i + 1);
...
void trocar_figurinha(char repetida[], Figurinha* figurinha, Album* album, int quant_fig){
   ...
   if(probabilidade>0 && probabilidade<=3){
      ...
      for(int i=0; i<quant_fig; i++){
            if(strcasecmp(figurinha[i].codigo, repetida)!=0){
               album->fig_coladas[album->posicao_no_album] = figurinha[i];
               album->posicao_no_album++;
               for(int j = 0; j < album->posicao_repetidas_ou_nao_coladas; j++){
                  if(strcasecmp(album->fig_repetidas_ou_nao_coladas[j].codigo,repetida)==0){
                     strcpy(album->fig_repetidas_ou_nao_coladas[j].codigo,"00000");
                  }
               }
               for(int k = 0, l = 1; k < album->posicao_repetidas_ou_nao_coladas; k++, l++){
                  if(strcasecmp(album->fig_repetidas_ou_nao_coladas[k].codigo,"00000")==0){
                     album->fig_repetidas_ou_nao_coladas[k]=album->fig_repetidas_ou_nao_coladas[l];
                     strcpy(album->fig_repetidas_ou_nao_coladas[l].codigo,"00000");
                  }
               }
